...
hbb_name = 'yqDesk' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/yqDesk.exe')
    os.chdir('../..')
    if os.path.exists('./yqDesk_portable.exe'):
        os.replace('./target/release/yqDesk-portable-packer.exe',
                   './yqDesk_portable.exe')
    else:
        os.rename('./target/release/yqDesk-portable-packer.exe',
                  './yqDesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/yqDesk_portable.exe')
    os.rename('./yqDesk_portable.exe', f'./yqDesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/yqDesk-{version}-install.exe')
